...
def get_entity_embeddings(
    ...
    for idx in range(num_keywords):
        keyword_idx_embeddings = np.stack(entity[f"keyword_{idx}_embedding"].apply(ast.literal_eval))
    ...
    entity_embeddings = np.concat(
        [function_name_embeddings, definition_embeddings, description_embeddings, keyword_embeddings],
        axis=1
    )
...
def retrieve(
    ...
    if os.path.isfile(query_path):
        ...
        query_prompt = keyword_extraction_prompt_template.format(description=query, code="")
        query_keywords = get_together_chat_response(prompt=query_prompt,
        ...
        for query_keyword_idx, query_keyword in enumerate(query_keywords):
            query_keyword_emb = get_together_embedding(text=query_keyword,
    ...
    score = pd.DataFrame.from_dict(data={"embedding": emb_scores.tolist(), "bm25": bm25_scores}).reset_index()

    # first getting the top-5 BM25 search scores, then return the top embedding scores among those top-5 scores
    result_idx = int(
        score.sort_values(by="bm25",
                          ascending=False).iloc[:9].sort_values(by="embedding", ascending=False).iloc[0]['index']
    )
    result = entity.iloc[result_idx][["id", "function_name", "description", "definition"]].to_dict()
    return result
